...
if ($type == 1) {
    ...
    $sql = "SELECT account_head, sum(income) as income, sum(expenditure) as expenditure, sum(amount) as amount,  date 
        FROM cashbook 
        WHERE date BETWEEN '$date_from' AND '$date_to' AND sccode='$sccode'
        GROUP BY date,  account_head
        ORDER BY date , account_head";
