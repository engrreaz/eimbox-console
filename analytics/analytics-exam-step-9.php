...
SELECT
    ...
    COUNT(DISTINCT CASE WHEN COALESCE(ss.fourth, 0) = 0 AND (sm.presence = 0 OR (sm.markobt / NULLIF(sm.fullmark, 1) * 100) < 33 OR sm.gp <= 0 OR sm.gp IS NULL) THEN sm.subject END) AS failed_subjects,
    ...
    GROUP_CONCAT(DISTINCT CASE WHEN COALESCE(ss.fourth, 0) = 0 AND (sm.presence = 0 OR (sm.markobt / NULLIF(sm.fullmark, 1) * 100) < 33 OR sm.gp <= 0 OR sm.gp IS NULL) THEN sm.subject ELSE NULL END ORDER BY sm.subject SEPARATOR ', ') AS failed_subject_codes,
    GROUP_CONCAT(DISTINCT CASE WHEN COALESCE(ss.fourth, 0) = 0 AND (sm.presence = 0 OR (sm.markobt / NULLIF(sm.fullmark, 1) * 100) < 33 OR sm.gp <= 0 OR sm.gp IS NULL) THEN sub.subject ELSE NULL END ORDER BY sm.subject SEPARATOR ', ') AS failed_subject_names
FROM stmark sm
...
LEFT JOIN examlist el ON el.id = sm.examid
