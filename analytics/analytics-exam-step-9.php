...
SELECT
    ...
    COUNT(DISTINCT CASE WHEN (sm.presence = 1 OR sm.markobt > 0) AND COALESCE(ss.fourth, 0) = 0 AND ((sm.markobt / NULLIF(sm.fullmark, 1) * 100) < 33 OR sm.gp <= 0 OR sm.gp IS NULL) THEN sm.subject END) AS failed_subjects,
