...
$campaign = mysqli_real_escape_string($conn, $_POST['campaign'] ?? 'Bulk Campaign');
$sms_type = mysqli_real_escape_string($conn, $_POST['sms_type'] ?? 'notice');
...
foreach ($recipients as $r) {
    ...
    $insert_rows[] = "('$sccode', '$sessionyear', '$rec_type_esc', '$rec_id_esc', '$name_esc', '$cls_esc', '$sec_esc', $roll, '$mobile_esc', '$sms_type', '$campaign', '$text_esc', $len, $parts, $parts, $is_unicode, 'queued', '$batch_id', '$usr', NOW())";
}

if (empty($insert_rows)) {
    echo json_encode(['status' => 'error', 'message' => 'No valid recipient numbers found!']);
    exit;
}
...
foreach ($chunks as $chunk) {
    $sql = "INSERT INTO sms 
        (sccode, sessionyear, recipient_type, recipient_id, recipient_name, classname, sectionname, rollno, mobile_number, sms_type, campaign, sms_text, sms_len, sms_parts, count, is_unicode, status, batch_id, send_by, send_time) 
