...
function parse_line($line){
    ...
    if (preg_match('/PHP\s+([A-Z]+)/', $res['msg'], $t)) $res['type'] = $t[1];
    if (preg_match('/in\s+(.*?)\s+on\s+line/', $line, $f)) $res['file'] = basename($f[1]);
    return $res;
}
...
if ($action === 'fetch'){
    ...
    if (empty($params['csrf']) || !csrf_ok($params['csrf'])) {
        http_response_code(400); echo json_encode(['ok'=>false, 'msg'=>'Invalid CSRF']); exit;
    }
    ...
    foreach ($lines as $i => $ln) {
        $p = parse_line($ln);
        if ($filter_date && strpos($p['time'], $filter_date) === false) continue;
        if ($filter_type && $filter_type !== $p['type']) continue;
        if ($filter_file && stripos($p['file'], $filter_file) === false) continue;
        if ($filter_text && stripos($p['msg'], $filter_text) === false) continue;
        $items[] = $p;
    }
...
if ($action === 'remove_by_index'){
    ...
    $csrf = $params['csrf'] ?? '';
    if ($idx === null || !csrf_ok($csrf)) { echo json_encode(['ok'=>false,'msg'=>'Invalid request']); exit; }

    $lines = array_reverse(read_lines($LOG_FILE)); // newest first
    if (!isset($lines[$idx])) { echo json_encode(['ok'=>false,'msg'=>'Index not found']); exit; }
...
if ($action === 'download'){
    ...
    if (!csrf_ok($csrf)) { http_response_code(403); echo "Invalid CSRF"; exit; }
    if (!file_exists($LOG_FILE)) { http_response_code(404); echo "No log"; exit; }
...
if ($action === 'remove'){
    $csrf = $params['csrf'] ?? '';
    if (!csrf_ok($csrf)) { echo json_encode(['ok'=>false,'msg'=>'Invalid CSRF']); exit; }
    $line = $params['line'] ?? '';
    if ($line === '') { echo json_encode(['ok'=>false,'msg'=>'Empty line']); exit; }
    $lines = read_lines($LOG_FILE);
    $new = [];
    foreach ($lines as $l) if (trim($l) !== trim($line)) $new[] = $l;
    file_put_contents($LOG_FILE, implode("\n",$new));
    echo json_encode(['ok'=>true]);
    exit;
}
...
if ($action === 'clear'){
    ...
    if (!defined('LOG_CLEAR_PASSWORD') || LOG_CLEAR_PASSWORD === 'change_me') {
        echo json_encode(['ok'=>false,'msg'=>'Clear password not configured. Update log-api.php LOG_CLEAR_PASSWORD.']); exit;
    }
    if ($given !== LOG_CLEAR_PASSWORD){
        echo json_encode(['ok'=>false,'msg'=>'Wrong password']); exit;
    }
...
if ($action === 'auto_archive'){
    if (!is_dir($ARCHIVE_DIR)) mkdir($ARCHIVE_DIR,0755,true);
    if (!file_exists($LOG_FILE)) { echo json_encode(['ok'=>false,'msg'=>'No log']); exit; }
