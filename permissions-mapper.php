...
<div class="container-xxl flex-grow-1 container-p-y">
    ...
    <h3>Access Management Tool</h3>

    <?php
    $foundmod = 0;
    foreach ($module_list as $module) {

        if (is_array($subscribe_module_list) && in_array($module['module_name'], $subscribe_module_list)) {
            $module_block = '';
        ...
        ?>
        <div class="module" <?php echo $module_block; ?>>

            <h5 class=" fw-bold text-primary"><?php echo $module['module_name']; ?></h5>
            ...
            <?php foreach ($sub_module_list as $sub_module) {
                if ($sub_module['module_name'] == $module['module_name']) {
                    ...
                    <div class="sub-module ">
                        ...
                        <h6><?php echo $sub_module['module_topic']; ?></h6>
                        <button class="custom-permission btn btn-warning">Add Custom Permission</button>
                        ...
                        foreach ($user_level_list as $u) {
                            ...
                            ?>
                            <div class="role-box row">

                                <div class="col-md-4 pt-2 ps-5">
                                    <?php echo $u['userlevel']; ?>
                                </div>

                                <div class="col-md-6">
                                    <select class="permission-select select form-select-sm" data-id="<?php echo $p['id']; ?>"
                                ...
                                <div class="col-md-2">
                                    <?php if ($cur_sccode > 100000) { ?>
                                        <button class="remove-permission btn btn-danger" data-id="<?php echo $p['id']; ?>">Set
                                            Default</button>
                                    <?php } ?>
                                </div>
                        ...
                        foreach ($permission_map as $p) {
                            if ($p['page_name'] == $page_name && !empty($p['email'])) {
                                ...
                                <hr class="m-0">
                                <div class="role-box">
                                    <h6 style="display:inline-block; color: dark; margin:0;"><?php echo $p['email']; ?> (Custom)
                                        <?php echo $p['id']; ?>
                                    </h6>
                                    <select class="permission-select form-select-sm p-2" data-id="<?php echo $p['id']; ?>"
                                    ...
                                    <button class="remove-permission btn btn-danger btn-sm"
                                        data-id="<?php echo $p['id']; ?>">Remove</button>
                                </div>
                                <?php
                            }
                        }
    ...
    if ($foundmod == 0) {
        ?>
        <div class="alert alert-danger">No Module Assigned.</div>
    <?php } ?>

    <!-- Modal HTML -->
    <div id="customModal">
        <h5>Select User for Custom Permission</h5>
        <select id="customUserSelect" class="select form-control">
            <option value="">-- Select User --</option>
        </select>
        <div class="mt-3">
            <button id="addCustomPermissionBtn" class="btn btn-outline-primary">Add</button>
            <button id="closeModalBtn" class="btn btn-outline-danger">Close</button>
        </div>
    ...
    $(document).on('click', '.custom-permission', function () {
        ...
        $.ajax({
            ...
            success: function (users) {
                ...
                users.forEach(u => select.append('<option value="' + u.email + '">' + u.profilename + ' (' + u.email + ')</option>'));
            },
