...
<script>
    ...
    $('#startAnalysisBtn').on('click', async function () {
        ...
        try {
            ...
            const response = await fetch('analytics/analysis_steps.json?v=' + new Date().getTime());
            if (!response.ok) throw new Error('Could not load analysis_steps.json');
            const steps = await response.json();
            
            // 2. Execute each step sequentially
            for (const step of steps) {
                updateProgress(currentProgress, step.icon || 'bi-gear', `Starting: ${step.title}...`);
                ...
                updateProgress(currentProgress, step.icon || 'bi-gear', `Completed: ${step.title}`);
            }
